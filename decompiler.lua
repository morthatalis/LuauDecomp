...
function module.decompileluac(bytecode)
	...
	local function addlocal(valuetocheck:string)
		...
		if strtotype(returnvalue(valuetocheck)) == nil then
			return "local "
		else
			return ""
		end
	...
	for i, line in ipairs(lines) do
		...
		if skipline == false then
			if op == "JUMPIFNOT" then
				...
				if string.match(arg2,"%[") and arg2 ~= nil then
					if iselseif == false then
						retval = retval .. "if " .. returnvalue(arg1) .. " then"
			...
			elseif op == "JUMPIF" then
				...
				if string.match(arg2,"%[") and arg2 ~= nil then
					if iselseif == false then
						retval = retval .. "if not " .. returnvalue(arg1) .. " then"
						jumpindexes[i] = tonumber(string.sub(arg2,3,#arg2-1)) + 1
					else
						retval = retval .. "elseif not " .. returnvalue(arg1) .. " then"
					end
				end
			...
			elseif op == "RETURN" then
				...
				elseif i == #lines then
					...
					retval = retval .. "--this is a random return (from what i know)" 
				end
				
			elseif op == "LOADNIL" then
				local arg1 = word[2]
				
				retval = retval ..addlocal(arg1) ..  arg1 .. " = nil"
			...
			elseif op == "LOADB" then
				...
				retval = retval .. addlocal(arg1) ..  arg1 .. " = " .. tostring(bool)
				reg[arg1] = bool
				creg[arg1] = bool
			...
			elseif op == "GETIMPORT" then
				...
				importname = string.sub(arg2,2,#arg2 - 1)
					...
					requirecall = ""
					--noline = true
					retval = retval .. addlocal(arg1)..  arg1 .. " = " .. string.sub(arg2,2,#arg2 - 1)
				end
			...
			elseif op == "GETTABLEKS" then
				...
				if requirebool ~= true then
					print(arg2, i, line)
					if isstr(creg[arg2]) then
					if string.sub(creg[arg2], 1,8) ~= "require" then
						reg[arg1] =  "RT"..arg2 ..arg3 
						if creg ~= nil then
							creg[arg1] =  tostring(creg[arg2]) ..arg3
						elseif reg[arg2] ~= nil then
							creg[arg1] =  tostring(reg[arg2])  ..arg3
						else
							creg[arg1] =  tostring(arg2) .. arg3
						end
					end
					end
			...
			elseif op == "CALL" then
				...
				if B >= 1 then
					for i = 1, B do
						if i == 1 and reg[ "R" .. tostring(A + i)] == nil then
							continue
						end
						local a =  "R" .. tostring(A + i)
						local regen = tostring(reg[a])
						...
						if reg[a] ~= nil and reg[a] ~= "nil" and reg[a] ~= "{}" then
							...
							table.insert(args, tostring(reg[ "R" .. tostring(A + i)]))
						else
			...
			elseif op == "JUMPIFLT" then
				...
				if string.match(arg2,"%[") and arg2 ~= nil then
					if iselseif == false then
						retval = retval .. "if " .. returnvalue(arg1) .. " >= " .. returnvalue(arg3) .. " then"
						jumpindexes[i] = tonumber(string.sub(arg2,3,#arg2-1)) + 1
						currentjumpindex[i] = i
					else
						retval = retval .. "elseif " .. returnvalue(arg1) .. " >= " .. returnvalue(arg3) .. " then"
					end
			...
			elseif op == "JUMPIFEQKS" then
				...
				if string.match(arg2,"%[") and arg2 ~= nil then
					if iselseif == false then
						retval = retval .. "if " .. returnvalue(arg1) .. " == not " .. returnvalue(arg3) .. " then"
						jumpindexes[i] = tonumber(string.sub(arg2,3,#arg2-1)) + 1
						currentjumpindex[i] = i
					else
						retval = retval .. "elseif " .. returnvalue(arg1) .. " == not " .. returnvalue(arg3) .. " then"
					end
				end
			...
			elseif op == "CAPTURE" then
				...
				local comment = "--captured on line: " .. i
				local rem = ""
 ...
 FASTCALL1 MATH_ABS R5 [+2] <-- +2 shows that this call will be called in 2 instructions
  ...
  CALL R4 1 1 <-- r4 is the same register as the one in the imported call
			...
			elseif op == "GETGLOBAL" then
				local arg1 = word[2]
				local arg2 = string.sub(word[4],2, #word[4]-1)
				retval = retval .. addlocal(arg1).. arg1 .." = _G[" .. arg2 .. "]"
			elseif op == "GETVARARGS" then
				retval = retval .. addlocal(arg1) .. word[2] .. " = {...}"
